...
- **Bitmap-ondertitels** — tracks die uit plaatjes bestaan (blu-ray-PGS, dvd-VobSub) bestaan voor mpv noch hls.js: de server levert ze als cue-index plus sprite-sheets (`/hls/{id}/bsub_{streamId}.json`), en `BitmapSubtitleOverlay` tekent ze in de controls-stack — de enige ondertitelrenderer die de app zelf bezit, identiek op elk platform. `MediaPlayerHandler.bitmapSubtitle` houdt de keuze vast (sluit een ondertiteltrack van de player uit, blijft behouden bij het heropenen van hetzelfde bestand, doet mee in de taalvoorkeur waarbij tekst bij gelijke taal wint). De overlay extrapoleert de positie met een `Ticker` (web meldt die maar elke ~250 ms), houdt hooguit drie gedecodeerde sheets vast, en rekent cue-coördinaten om via de actuele `BoxFit` en de toegepaste mpv-crop (`appliedVideoCrop`). De geometrie en het opzoeken van cues in `lib/utils/subtitles/BitmapSubtitles.dart` zijn puur en hebben unittests.

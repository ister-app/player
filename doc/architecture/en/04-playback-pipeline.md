...
- **Bitmap subtitles** — picture-based tracks (Blu-ray PGS, DVD VobSub) exist for neither mpv nor hls.js: the server serves them as a cue index plus sprite sheets (`/hls/{id}/bsub_{streamId}.json`), and `BitmapSubtitleOverlay` draws them in the controls stack — the one subtitle renderer the app owns, identical on every platform. `MediaPlayerHandler.bitmapSubtitle` holds the selection (mutually exclusive with a player subtitle track, kept across a re-open of the same file, part of the language preference with text winning a tie). The overlay extrapolates the position with a `Ticker` (web reports it only every ~250 ms), keeps at most three decoded sheets, and maps cue coordinates through the live `BoxFit` and the applied mpv crop (`appliedVideoCrop`). The geometry and cue lookup in `lib/utils/subtitles/BitmapSubtitles.dart` are pure and unit tested.

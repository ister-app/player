...
macOS-builds zijn nieuw en nog experimenteel. De makkelijkste route is **TestFlight**: installeer Apples [TestFlight-app](https://apps.apple.com/app/testflight/id899247664) uit de Mac App Store (vereist macOS 12 of nieuwer), accepteer de uitnodiging van degene die je server beheert (of van het project), en Ister installeert en updatet als elke andere app — geen Terminal, geen waarschuwingen.

Als alternatief download je `player-macos-<versie>.zip` uit de [nieuwste release](https://github.com/ister-app/player/releases), pak je die uit en sleep je `Ister player.app` naar je map Programma's. Die build is niet door Apple genotariseerd, dus macOS blokkeert hem bij de eerste start; haal de download-quarantaine er eenmalig af via Terminal:
...
De iPhone- en iPad-versie wordt verspreid via **TestFlight**: installeer Apples [TestFlight-app](https://apps.apple.com/app/testflight/id899247664), accepteer de uitnodiging van degene die je server beheert (of van het project), en Ister installeert en updatet als elke andere app — geen Mac, geen kabel. Nieuwe releases verschijnen vanzelf in TestFlight.

Zelf bouwen blijft mogelijk voor ontwikkelaars. Je hebt een Mac nodig met [Xcode](https://apps.apple.com/app/xcode/id497799835) en de [Flutter SDK](https://docs.flutter.dev/get-started/install/macos), plus een Apple ID.
...
Met een gratis Apple ID werkt een zelfgebouwde app na **zeven dagen** niet meer en moet je opnieuw bouwen; met een betaald Apple Developer Program-lidmaatschap is dat een jaar. De TestFlight-route kent geen van beide beperkingen.

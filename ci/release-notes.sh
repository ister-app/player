#!/usr/bin/env bash
# Build the release notes for a player release.
#
...
# wins — which silently produces .../player.git/commit/<sha> links that 404.
REPO="${GITHUB_REPOSITORY:-$(git remote get-url origin 2>/dev/null |
  sed -E -e 's#\.git$##' -e 's#^.*[:/]([^/]+/[^/]+)$#\1#' || echo ister-app/player)}"

{
  echo "## player v${VERSION}"
  echo
  echo "The web player ships as an image; the Android and Linux builds are attached to this release."
  echo
  echo "| Platform | Where |"
  echo "|---|---|"
  echo "| Web | \`ghcr.io/ister-app/player:${VERSION}\` |"
  echo "| Android | \`app-release.apk\` below |"
  echo "| Linux | \`app.ister.Player.flatpak\` below |"
  echo
  ...
  echo
  echo '```sh'
  echo "docker pull ghcr.io/ister-app/player:${VERSION}"
  echo '```'
  echo
  if [ -n "$PREV" ]; then

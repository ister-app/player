...
authoritative pins are the four `Package.swift` files (macos/ios × video/audio; the variant slug
differs per manifest — `macos-universal-audio-full` but `ios-universal-audio-default`, and
`-video-default` for both — so read each file's `libmpvArtifactBase` rather than assuming) —
each xcframework zip by url + sha256. The release assets' own `digest` field (`gh api
repos/ister-app/libmpv-darwin-build/releases/tags/<tag>` → `.assets[].digest`) is exactly the
sha256 SwiftPM wants, so a bump needs no downloads. The CocoaPods Makefiles carry a
...
- **iOS resolves libmpv through SwiftPM**, which is on by default on stable and is what the
  `ios/` project relies on: `Package.swift` pins each xcframework by url + sha256, so a bad
  download fails the build. The CocoaPods path does not — `media_kit_libs_ios_video`'s podspec
  runs `system("make")`, whose failures are swallowed, after which `media_kit_video` compiles
...
This is a Flutter media player app for the "Ister" media management system. It supports multiple servers with OIDC authentication; browsing and playback of episodes, movies, music, podcasts and audiobooks/epubs; and language preferences and watch-progress tracking. It ships for Android (incl. Android TV and Android Auto), Linux (flatpak), Windows, macOS, iOS and web.
...
- `ServerStore` — The configured server list (SharedPreferences `servers`) with `add`/`remove` (remove also drops well-known, login, stream token and client). `AddServerPage` (`/add-server`, root router) is the guided add flow: `normalizeServerInput` (`ServerAddress.dart`) strips scheme/trailing slash, `WellKnownService.probe` distinguishes found / not-an-Ister-server / unreachable, and nothing is saved until the user confirms the discovered name. `ServerList` (the overview) renders the welcome state when empty; `test/ux_screenshots_test.dart` renders these screens to PNG when `UX_SCREENSHOT_DIR` is set.
...
- `GraphQLCacheStore` — Disk-backed `Store`s for the per-server `GraphQLCache` (Hive, one box per server under `<app support>/graphql_cache`, opened in `main` before any client is built), so a restart repaints the last browsed content instead of empty shelves. Only helps the `cacheAndNetwork` pages; services deliberately stay `networkOnly`. Boxes are stamped with the app version and dropped on an upgrade (a new build may select different fields) or past an entry cap, and a box that fails to open falls back to `InMemoryStore` — the cache is an optimisation, never a correctness requirement.
...
- `UserSettingsService` — The user's playback settings (spoken/subtitle languages, direct play, transcode, max video height) as the **server** stores them, per user per server, with a local cache. `LanguagePreferences` and `PlaybackPreferences` are thin read/write facades over it — call those, not the GraphQL mutation. Defaults are direct play **on**, transcode **off** (server-side `UserSettingsService.defaults()`, mirrored by the client's fallbacks); web overrides both at playback (`kIsWeb` in `MediaPlayerHandler._resolveMediaUrl`) and shows both switches fixed and disabled, because a browser cannot direct play. `LanguageService` supplies the ISO 639-3 language table from `assets/`.
...
- **Bitmap subtitles** (`lib/utils/subtitles/`, `lib/components/video_controls/BitmapSubtitleOverlay.dart`) — Blu-ray PGS / DVD VobSub tracks are pictures; the server serves them as `/hls/{mediaFileId}/bsub_{streamId}.json` (cue index, raw source-timeline milliseconds — no `sub-delay`-style offset) plus PNG sprite sheets, and the app draws them itself in the controls stack, the same way on every platform. Neither mpv nor hls.js ever sees these tracks: the selection lives in `MediaPlayerHandler.bitmapSubtitle` (setting it turns the player's own subtitle off; `switchSubtitleTrack` clears it), candidates come from the server's stream rows (`BitmapSubtitleTrack.of`, codecs the server has a parser for — not DVB). Offline, the index file in the download dir *is* the record that a track was mirrored (no manifest field). Keep the geometry (`canvasRectFor`, `clampInto`, `destinationFor`) and cue lookup pure — they are what the unit tests cover; the overlay widget itself only wires position, sheets and paint.
- **Downloads / offline** (`lib/utils/download/`) — Client-only, no server endpoints: `HlsDownloader` mirrors a media file's HLS tree (master + chosen variant/audio playlists + every segment, `?token=` stripped, server file names kept) into `<app support>/downloads/<server-slug>/<mediaFileId>/`, in playlist order with backoff because the server generates segments lazily (a GET blocks ≤60 s). `DownloadStore` keeps a JSON manifest per server; each `DownloadEntry` stores the JSON of a `Fragment$fragmentPlayQueue$playQueueItems` as its offline snapshot, so `LocalPlayQueue.build` rebuilds a queue with `fromJson` and every queue consumer works unchanged. `DownloadService` (singleton, `ensureStarted` in `main.dart`) runs the queue and answers `localMasterFor` synchronously for `MediaPlayerHandler._resolveMediaUrl`, the single place the player picks local-vs-server; `_restampToken`/subtitle side-loading short-circuit on `isLocalMediaUrl`. Queues with a `local:` id (`isLocalQueue`) have no client: progress goes to `OfflineProgressStore` and `OfflineSyncService.trySync` replays it as one `createPlayQueue` + `updateProgress` per item when the shell connects. The music cache (`PlayHistoryStore` records tracks after 30 s/50 % played; `planMusicCache` is a pure planner; `MusicCacheService` evicts non-pinned entries) never touches pinned (manual) downloads. Episodes sharing one media file (`EpisodeParts.bounds`) share its directory — `remove` only deletes it with the last entry, a sibling adopts the first one's result. Epub/CBZ/PDF files are `DownloadKind.book` entries (no queue item): `EpubDownloader` mirrors the manifest resources the reader uses (no CSS/fonts; audio+SMIL only for a read-aloud edition), `ComicDownloader` the pages or the whole pdf; `EpubResourceClient.localDir`, `LocalCbzPageSource` and `PdfPageSource.openFile` read them, reached through `OfflineReaderRoute`/`OfflineComicReaderRoute` on the root router. Reading positions the server rejects go to `ReadingProgressOutbox` (replayed by `OfflineSyncService`, and used as the resume position while offline). Readers never *wait* for the store (`localReadingDir` returns null until `ensureStarted` resolved the root — `path_provider` never completes under `flutter test`). On Android a `dataSync` foreground service (`DownloadForegroundService`, flutter_foreground_task, no task isolate) pins the process while downloads run; `DownloadForegroundController` derives start/update/stop from the service notifiers. iOS has no equivalent — BGTaskScheduler grants ~30 s per ~15 min, useless for an HLS tree — so `DownloadWakelock` reuses the same controller to hold the screen awake instead, downloads only progress while the app is open, and `DownloadsPage` says so. That wakelock goes through `ScreenWakelock`, which reference-counts `WakelockPlus` because the epub and comic readers hold the same global flag. The Downloads pages live on the root router (`/downloads/:serverName`) so they render without the server shell. `PodcastPage`'s server-side `downloadPodcastEpisode` is worded "fetch to server" to keep `download` for the device.
...
- **Artwork sizing:** network artwork goes through `ArtworkImage` (`lib/components/ArtworkImage.dart`), never a bare `CachedNetworkImage` — a lint test in `test/image_cache_key_lint_test.dart` enforces it. It appends `?width=` off `ArtworkSizing.widthBuckets` (mirrored by `WIDTH_BUCKETS` in the server's `FileController`; an older server ignores the parameter and returns the original) and, **on native only**, caps the decode with `memCacheWidth`. Web gets no decode cap on purpose: under its default `HtmlImage` loader `memCacheWidth` is a **silent no-op** (`ui_web.createImageCodecFromUrl` drops the decode callback), and the `HttpGet` loader that would honour one — through `maxWidthDiskCache`, never together with `memCacheWidth`, which trips `ResizeImage`'s `getTargetSize == null` assert — routes images through the cache manager instead of the browser's `<img>` and **left every tile blank in Firefox**. The request-side sizing is what keeps web small. `ArtworkImage.providerFor` is the same thing for callers with no layout to measure (the blurred player backdrop, epub images).
- **Blank tiles on web:** CanvasKit on a GPU builds a *lazy* SkImage holding the `<img>` it decoded from and uploads the texture at paint time; Firefox reclaims the decoded data of a detached `<img>`, and the upload then writes zeros (`WebGL warning: Resource has no data (yet?). Uploading zeros`). Feeding it bytes does not avoid this — without WebCodecs' `ImageDecoder` the engine wraps them in a blob `<img>` (`canvaskit/image.dart`). `ImageCacheConfig` therefore sets `imageCache.maximumSize = 0` on web, so a stale image cannot survive a navigation and be repainted; live (on-screen) images are tracked separately and unaffected. If that ever proves too costly, the real fix is a web `ImageProvider` that decodes through `createImageBitmap` and `ui_web.createImageFromImageBitmap`, which is not `<img>`-backed.

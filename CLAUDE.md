...
- **Music player overlay:** `MusicPlayerRoute` is pushed on the root router as a transparent route. `MediaPlayerHandler.dismissMusicPlayer`/`musicPlayerOpen` coordinate with `_MusicPlayerAwareDelegate.popRoute` in `AppRouter.dart` so the back button plays the dismiss animation instead of popping instantly.

...
- **Artwork sizing:** network artwork goes through `ArtworkImage` (`lib/components/ArtworkImage.dart`), never a bare `CachedNetworkImage` — a lint test in `test/image_cache_key_lint_test.dart` enforces it. It appends `?width=` off `ArtworkSizing.widthBuckets` (mirrored by `WIDTH_BUCKETS` in the server's `FileController`; an older server ignores the parameter) and caps the decode with the knob that platform honours: `memCacheWidth` on native, `maxWidthDiskCache` under `ImageRenderMethodForWeb.HttpGet` on web. That difference is the whole reason the widget exists — on web's default `HtmlImage` loader `memCacheWidth` is a **silent no-op** (`ui_web.createImageCodecFromUrl` drops the decode callback), which is how one home screen came to decode 872 MiB and paint grey tiles in Firefox; and setting both knobs trips `ResizeImage`'s `getTargetSize == null` assert. `ArtworkImage.providerFor` is the same thing for callers with no layout to measure (the blurred player backdrop, epub images).
